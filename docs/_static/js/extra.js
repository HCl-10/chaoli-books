...
if (cur) {
  ...
  if (pos < 0)
    textLength =
      document
        .getElementsByClassName("md-content__inner")[0]
        .textContent.slice(0, pos)
        .replace(/\s/g, "").length -
      document.getElementsByTagName("h1")[0].textContent.replace(/\s/g, "")
        .length -
      document
        .getElementsByClassName("page-copyright")[0]
...
function foo(response) {
  ...
  if (data && data[0]) {
    var ti = new Date(data[0].commit.author.date).toLocaleString("zh-CN", {
      timeZone: "Asia/Beijing",
      hour12: false,
    });

    var author_list = document
      .getElementsByClassName("page_contributors")[0]
      .innerHTML.split(", ");
    author_list = author_list.filter((e) => {
      return e != "chaoli-books";
    });
    for (var i = 0; i < num; ++i) {
    ...
    document.getElementsByClassName(
      "page_contributors"
    )[0].innerHTML = sorted.join(", ");
  } else if (!url.endsWith("index")) {
    url += "/index";
    var script = document.createElement("script");
    document
      .getElementsByClassName("edit_history")[0]
      .setAttribute(
        "href",
        `https://github.com/HCl-10/chaoli-books/commits/master/docs/${url}`
      );
    script.src = `https://api.github.com/repos/HCl-10/chaoli-books/commits?path=docs/${url}&callback=foo`;
...
if (url == "/index.md") {
  ...
  document
    ...
    .setAttribute(
      "href",
      `https://github.com/HCl-10/chaoli-books/commits/master/docs/index.md`
    );
  script.src = `https://api.github.com/repos/HCl-10/chaoli-books/commits?path=docs/index.md&callback=foo`;
  document.getElementsByTagName("head")[0].appendChild(script);
  ...
  document
    .getElementsByClassName("edit_history")[0]
    .setAttribute("href", `https://github.com/HCl-10/chaoli-books/commits/master`);
  document.getElementsByClassName("facts_modified")[0].innerHTML =
    "最近没更新过这个页面";
  document.getElementsByClassName("page_contributors")[0].innerHTML =
    "本页面是自动生成的";
